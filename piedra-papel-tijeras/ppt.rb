...
get '/throw/:type' do
 ...
 halt 403, "Error: You must throw one of the following: #{@throws}"
 end
 # now we can select a random throw for the computer
 computer_throw = @throws.sample
 # compare the player and computer throws to determine a winner
 if player_throw == computer_throw
 "Empate con la maquina. Intenta otra vez!"
 elsif computer_throw == @defeat[player_throw]
 "Bien hecho; #{player_throw} derrota a la maquina #{computer_throw}!"
 else
 "Ouch; #{computer_throw} beats #{player_throw}. Mejor suerte la proxima vez!"
 end
end
